...
class PrintProcess extends _$PrintProcess {
  ...
  FutureOr<void> _startPrinting() async {
    ...
    try {
      ...
      // 3. 프린트 작업 생성
      final printJobInfo = await _createPrintStatus(
  ...
  Future<void> _handlePrintFailure(
    ...
    logger.e('Print failure', error: error, stackTrace: stack, time: DateTime.now());
  ...
  Future<({int printedPhotoCardId})> _createPrintStatus(
